...
class PubMedQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"gen_enc": {k: v.squeeze(0) for k, v in gen_enc.items()}, "labels": labels.squeeze(0)}
...
def evaluate_central(test):
    ...
    gen_tokenizer = T5Tokenizer.from_pretrained(GENERATOR_MODEL)
    correct, total = 0, 0
    for batch in loader:
        outs = gen.generate(input_ids=batch["gen_enc"]["input_ids"])
        for o, l in zip(outs, batch["labels"]):
            if gen_tokenizer.decode(o, skip_special_tokens=True) == gen_tokenizer.decode(l, skip_special_tokens=True):
                correct += 1
            total += 1
...
def main():
    ...
    for m in methods:
        if hasattr(histories[m], 'losses_distributed') and histories[m].losses_distributed:
            losses = [loss for round_losses in histories[m].losses_distributed for loss in round_losses]
            plt.plot(range(1, len(losses) + 1), losses, marker='o', label=m)
    plt.xlabel("Round")
    plt.ylabel("Avg. Loss")
    plt.title("Loss per Round")
    plt.legend()
    plt.savefig("federated_rag_results.png")
    plt.show()
